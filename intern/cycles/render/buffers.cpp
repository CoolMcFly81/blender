...
int BufferParams::get_passes_size()
{
	...
	if(denoising_passes) {
		/* Feature passes: 11 Channels (3 Color, 3 Normal, 1 Depth, 4 Shadow) + 9 Variance + 2 Shadow Sample
		 * Color passes: 3 Noisy (RGB) + 3 Variance [+ 3 Skip (RGB)] */
		size += selective_denoising? 30: 27;
	}

	size = align_up(size, 4);

	size += num_light_groups*4;
...
bool RenderBuffers::get_denoising_rect(int type, float exposure, int sample, int components, int4 rect, float *pixels, bool read_pixels, int frame)
{
	...
	if(type & EX_TYPE_DENOISE_ALL) {
		...
		switch(type) {
			...
			case EX_TYPE_DENOISE_SHADOW_A:   type_offset = 14; break;
			case EX_TYPE_DENOISE_SHADOW_B:   type_offset = 18; break;
			case EX_TYPE_DENOISE_NOISY:      type_offset = 21; scale = exposure/sample; break;
			case EX_TYPE_DENOISE_NOISY_VAR:  type_offset = 24; scale = exposure*exposure/sample; break;
			case EX_TYPE_DENOISE_CLEAN:      type_offset = 27; scale = exposure/sample; break;
		}
		...
		if(components == 1) {
			...
			assert(type & (EX_TYPE_DENOISE_SHADOW_A));
			...
			assert(components == 3);
			assert(!(type & (EX_TYPE_DENOISE_DEPTH | EX_TYPE_DENOISE_DEPTH_VAR | EX_TYPE_DENOISE_SHADOW_A)));
	...
	else if(type >= EX_TYPE_LIGHT_GROUP_1 && type <= EX_TYPE_LIGHT_GROUP_8)
	{
		...
		float *in = (float*)buffer.data_pointer + params.get_light_groups_offset();
		int pass_stride = params.get_passes_size();
		in += params.width*params.height*pass_stride * frame;
