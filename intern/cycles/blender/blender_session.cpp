...
#undef LIGHT_GROUP
		...
		if(b_layer_iter->keep_denoise_data()) {
			...
			add_pass(b_engine, SCE_PASS_DENOISE_SHADOW_A, 4, b_rlay_name.c_str(), NULL);
...
void BlenderSession::get_progress(float& progress, double& total_time, double& render_time)
{
	...
	long pixel_samples = session->progress.get_pixel_samples();
	long total_samples = session->tile_manager.get_total_pixel_samples();

	if(background || session->tile_manager.get_num_effective_samples() != INT_MAX) {
		progress = (total_samples > 0)? ((float) pixel_samples) / total_samples : 0;
	}
	else {
		progress = 0.0f;
	}
...
{
	long pixel_samples = session->progress.get_pixel_samples();
	long total_samples = scene->bake_manager->total_pixel_samples;

	float progress = (total_samples > 0)? ((float) pixel_samples) / total_samples : 0;
