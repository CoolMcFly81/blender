...
ccl_device float kernel_branched_path_integrate(KernelGlobals *kg, RNG *rng, int sample, Ray ray, ccl_global float *buffer, PathRadiance *L, float3 *L_sum)
{
	/* initialize */
	...
	int sample, int x, int y, int offset, int stride)
{
	...
	/* integrate */
	PathRadiance L;
	float3 L_sum = make_float3(1e8f, 1e8f, 1e8f);

	if(ray.t != 0.0f) {
		float alpha = kernel_branched_path_integrate(kg, &rng, sample, ray, buffer, &L, &L_sum);
		kernel_write_result(kg, buffer, sample, &L, L_sum, alpha);
	}
	else {
		kernel_write_result(kg, buffer, sample, NULL, L_sum, 0.0f);
	}
