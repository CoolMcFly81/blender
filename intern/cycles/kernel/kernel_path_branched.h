...
ccl_device float kernel_branched_path_integrate(KernelGlobals *kg, RNG *rng, int sample, Ray ray, ccl_global float *buffer, PathRadiance *L, float3 *shadow_L)
{
	/* initialize */
...
#  endif
		...
		if(!(sd.flag & SD_HAS_ONLY_VOLUME)) {
			...
			/* direct light */
			if(kernel_data.integrator.use_direct_light) {
				float4 shadow_info[2];
				if(write_denoising_shadow) {
					shadow_info[0] = *((float4*) (buffer + kernel_data.film.pass_denoising + 14));
					shadow_info[1] = *((float4*) (buffer + kernel_data.film.pass_denoising + 18));
				}
				int all = (kernel_data.integrator.sample_all_lights_direct) ||
				          (state.flag & PATH_RAY_SHADOW_CATCHER);
				kernel_branched_path_surface_connect_light(kg, rng,
					&sd, &emission_sd, &hit_state, throughput, 1.0f, L, all, write_denoising_shadow? shadow_info : NULL);
				if(write_denoising_shadow) {
					*((float4*) (buffer + kernel_data.film.pass_denoising + 14)) = shadow_info[0];
					*((float4*) (buffer + kernel_data.film.pass_denoising + 18)) = shadow_info[1];
				}
			}
#endif
	...
	int sample, int x, int y, int offset, int stride)
{
	...
	/* integrate */
	PathRadiance L;
	float3 shadow_L = make_float3(-1.0f, -1.0f, -1.0f);

	if(ray.t != 0.0f) {
		float alpha = kernel_branched_path_integrate(kg, &rng, sample, ray, buffer, &L, &shadow_L);
		kernel_write_result(kg, buffer, sample, &L, shadow_L, alpha);
	}
	else {
		kernel_write_result(kg, buffer, sample, NULL, shadow_L, 0.0f);
	}
