...
ccl_device void kernel_filter_divide_shadow(KernelGlobals *kg, int sample, float **buffers, int x, int y, int *tile_x, int *tile_y, int *offset, int *stride, float *unfiltered, float *sampleVariance, float *sampleVarianceV, float *bufferVariance, int4 rect)
{
	...
	unfiltered[idx+Bofs] = center_buffer[19] / max(center_buffer[18], 1e-7f);
	float varFac = 1.0f / (center_buffer[17] * (center_buffer[17] - 1));
	sampleVariance[idx] = (center_buffer[16] + center_buffer[20]) * varFac;
	sampleVarianceV[idx] = 0.5f * (center_buffer[16] - center_buffer[20]) * (center_buffer[16] - center_buffer[20]) * varFac * varFac;
