...
ccl_device_inline void kernel_write_result(KernelGlobals *kg, ccl_global float *buffer,
	int sample, PathRadiance *L, float3 shadow_L, float alpha)
{
	if(L) {
		...
		/* Write shadowcatcher override? */
		if(shadow_L.x >= 0.0f) {
			kernel_write_pass_float4(buffer, sample, make_float4(shadow_L.x, shadow_L.y, shadow_L.z, alpha));

			kernel_write_light_passes(kg, buffer, L, sample);

			if(kernel_data.film.pass_denoising) {
				kernel_write_pass_float3_var(buffer + kernel_data.film.pass_denoising + 21, sample, shadow_L);
			...
			if(kernel_data.film.pass_denoising) {
				if(kernel_data.film.pass_no_denoising) {
					...
					kernel_write_pass_float3_var(buffer + kernel_data.film.pass_denoising + 21, sample, noisy);
					kernel_write_pass_float3_nopad(buffer + kernel_data.film.pass_no_denoising, sample, clean);
				}
				else {
					kernel_write_pass_float3_var(buffer + kernel_data.film.pass_denoising + 21, sample, L_sum);
				}
			}
		}
		...
		if(kernel_data.film.light_groups) {
			...
			for(int i = 0; i < 8; i++) {
				...
				kernel_write_pass_float4(buffer + ofs, sample, make_float4(L->light_groups[i].x, L->light_groups[i].y, L->light_groups[i].z, alpha));
				ofs += 4;
			}
		}
	}
		...
		if(kernel_data.film.pass_denoising) {
			kernel_write_pass_float3_var(buffer + kernel_data.film.pass_denoising + 21, sample, make_float3(0.0f, 0.0f, 0.0f));
		}
		if(kernel_data.film.pass_no_denoising) {
			kernel_write_pass_float3_nopad(buffer + kernel_data.film.pass_no_denoising, sample, make_float3(0.0f, 0.0f, 0.0f));
		}
